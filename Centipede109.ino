 /*
  ...
  working as of 6/15/22
  
  this one is designed to enroll and update chrome OS on a chromebook.
*/      
...
#define device_version 109 // Change to the ChromeOS version you expect to use with Centipede; Changes have been reported in the following ranges 58-68, 69, 70
...
#define wifi_name "WiFiSSIDHere" // Define SSID for your wireless connection.
#define wifi_pass "WiFiPasswordHere" // Define the password for your wireless connection.
#define wifi_security 2 //[0,1,2] Set to 0 for open, 1 for WEP, 2 for WPA
#define username "DeviceEnrollmentUser@domain.com" // Define the user name for enrolling the device.
#define password "SuperSecretPassword" // The password for the enrollment GAFE account.
#define username1 "GenericUser@domain.com" // Define the user name for logging into the device to update. Usually a generic tech/stu account.
#define password1 "SuperSecretPassword" // The password for the generic student account.
...
#define update_when_finished 1 //[0,1] Set to 1 if you want Centipede to update chrome OS after enrolling
...
#define chromeVox false //[true,false] Set false if your device doesn't ask for Chromevox
...
#define longer_enrollment_time 15 // Set to additional seconds to wait for Device Configuration and Enrollment
...
#define VERSION_109 (device_version >= 109)
...
void loop() { // Main Function - workflow is called within loop();
  if (digitalRead(buttonPin) == 1 ) { // Check for debugging. If not debugging, run the program
    showVersion();
    chromeVoxSkip(); // Skipping chromevox windows
    enterEnrollment();
    if (setWiFi) {
      wifiConfig(); // Enter the wifi configuration method (written down below)
      chromeVoxScreen(); // Skipping random chromevox windows
      welcomeScreen();
      wait(5);
    ...
    Keyboard.write(KEY_ENTER); // press OK to complete and go to login screen.
    
    if (update_when_finished) {
    wait(10 + longer_enrollment_time); // wait for Enrollment to complete
    enterAccount();
    wait(10 + longer_enrollment_time); // wait for desktop to load
    enterSettings(); // opens up settings
    updateChome();
    }
    showSuccess();
  }
...
void chromeVoxSkip() {
  // VOX messages randomly appears //Changed in v92+
  if (VERSION_91) {
    wait(5);
    Keyboard.press(KEY_LEFT_CTRL);
    Keyboard.press(KEY_LEFT_ALT);
...
void ToS() {
  ...
  switch(device_version) {
    ...
    case 86:
      ...
      break;
    case 109:
      // Remove Tab Key for Version 105 and Up
      //repeatKey(KEY_TAB, 4); //Move into ToS iFrame
      Keyboard.release(KEY_LEFT_SHIFT);
      n = 0;
      break;  
...
void updateChome() {
  ...
  Keyboard.write(KEY_ENTER);
  wait(5);
  repeatKey(KEY_TAB, 11); 
  Keyboard.write(KEY_ENTER);
  wait(2);
  Keyboard.write(KEY_TAB);
  Keyboard.write(KEY_ENTER); //hits update button.
}
